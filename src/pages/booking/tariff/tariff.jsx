...
const Tariff = () => {
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        enableSorting: true,
        cell: ({ row }) => <div className="p-2">{row.original.description}</div>,
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <input
            type="number"
            className="w-16 text-center border rounded p-1 justify-center"
            value={row.original.initialCharge}
            onChange={(e) =>
              handleInputChange(row.original.id, "initialCharge", e.target.value)
            }
          />
        ),
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <input
            ...
            onChange={(e) =>
              handleInputChange(row.original.id, "firstMileCharge", e.target.value)
            }
          />
        ),
        meta: { headerClassName: "w-20 text-center" },
      },
      {
        ...
        cell: ({ row }) => (
          <input
            ...
            onChange={(e) =>
              handleInputChange(row.original.id, "additionalMileCharge", e.target.value)
            }
          />
        ),
        meta: { headerClassName: "w-20 text-center" },
  ...
  return (
    <div className="container mx-auto mt-8 p-6">
      ...
      {/* Update Button */}
      <div className="mt-4">
        <Button className="bg-purple-600 hover:bg-purple-700 text-white font-semibold px-6 py-2 rounded" onClick={handleUpdateTariffs}>
          UPDATE TARIFF
        </Button>
      </div>
    </div>
